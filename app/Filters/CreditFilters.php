    ...
    public function filter(string $filter = ''): Builder
    {
        ...
        return  $this->builder->where(function ($query) use ($filter) {
            $query->where('credits.number', 'like', '%'.$filter.'%')
                          ...
                          ->orWhere('credits.custom_value5', 'like', '%'.$filter.'%')
                          ->orWhere('credits.custom_value6', 'like', '%'.$filter.'%')
                          ->orWhere('credits.custom_value7', 'like', '%'.$filter.'%')
                          ->orWhere('credits.custom_value8', 'like', '%'.$filter.'%')
