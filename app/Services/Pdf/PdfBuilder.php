    ...
    */
    ...
    public function transformLineItems($items, $table_type = '$product'): array
    {
        ...
        foreach ($items as $key => $item) {
            ...
            $data[$key][$table_type.".{$_table_type}5"] = strlen($item->custom_value5 ?? '') >= 1 ? $helpers->formatCustomFieldValue($this->service->company->custom_fields, "{$_table_type}5", $item->custom_value5 ?? '', $this->service->config->currency_entity) : '';
            $data[$key][$table_type.".{$_table_type}6"] = strlen($item->custom_value6 ?? '') >= 1 ? $helpers->formatCustomFieldValue($this->service->company->custom_fields, "{$_table_type}6", $item->custom_value6 ?? '', $this->service->config->currency_entity) : '';
            $data[$key][$table_type.".{$_table_type}7"] = strlen($item->custom_value7 ?? '') >= 1 ? $helpers->formatCustomFieldValue($this->service->company->custom_fields, "{$_table_type}7", $item->custom_value7 ?? '', $this->service->config->currency_entity) : '';
            $data[$key][$table_type.".{$_table_type}8"] = strlen($item->custom_value8 ?? '') >= 1 ? $helpers->formatCustomFieldValue($this->service->company->custom_fields, "{$_table_type}8", $item->custom_value8 ?? '', $this->service->config->currency_entity) : '';
